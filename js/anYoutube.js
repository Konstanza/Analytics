(function() {
  ...
  var OAUTH2_CLIENT_ID = '503363864277-k3pnrkheo7ppsks1aidnhtaeemd1kem4.apps.googleusercontent.com';
  var OAUTH2_SCOPES = [
  ...
  function getPlaylistItems(listId) {
    ...
    request.execute(function(response) {
      if ('error' in response) {
        displayMessage("Se requiere iniciar sesión");
      } else {
        if ('items' in response) {
          ...
          displayMessage('No hay videos en tu canal.');
        }
      }
    });
  }
    ...
    request.execute(function(response) {
      if ('error' in response) {
        ...
        $.each(response.items, function() {
          //Excluir videos que no tengan ninguna vista, ya que esos videos
          // no tendrá ningún número de datos de Analytics interesante
          ...
          aElement.attr('href', '#');
          aElement.text(title);
          displayMessage('Selecciona un video: ');
        ...
        if (videoList.children().length == 0) {
          ...
          displayMessage('Tu canal no tiene videos que hayan sido vistos.');
        }
      }
    });
  }
  ...
  function displayVideoAnalytics(videoId) {
    if (channelId) {
      ...
      var request = gapi.client.youtubeAnalytics.reports.query({
        ...
        metrics: 'views', matrics:'likes', metrics: 'dislikes' ,
      ...
      displayMessage('El ID de canal de YouTube para el usuario actual no está disponible.');
    }
  }
